...
public sealed class ModeService
{
    ...
    public string ActiveLabel => !HasSelection ? "STANDBY" : _modes[_state.SelectedModeIndex].Name.ToUpperInvariant();

    public int TotalProcesses => _modes.Sum(m => m.ProcessCount);
    public string ShortcutHint => Count == 0 ? "[1]" : Count == 1 ? "[1]" : $"[1-{Count}]";

    public ModeService(AppState state) => _state = state;
    ...
    public ModeConfig? Get(int index) => index >= 0 && index < _modes.Count ? _modes[index] : null;
